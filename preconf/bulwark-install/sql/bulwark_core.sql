...
INSERT INTO `x_settings` (`so_id_pk`, `so_name_vc`, `so_cleanname_vc`, `so_value_tx`, `so_defvalues_tx`, `so_desc_tx`, `so_module_vc`, `so_usereditable_en`) VALUES ('179', 'panel_le_extra_sans', 'SANs de servicio del cert del panel', '', NULL, 'Lista editable (CSV) de hostnames de servicio a incluir en el certificado Let''s Encrypt del panel (correo, ftp, ...). Es la fuente de verdad y se respeta en cada renovación; edítala para activar o quitar un nombre. Se siembra sola al instalar con el objetivo MX real del proveedor (create_base_zone), así el nombre del correo sale del propio DNS. El FQDN del panel y los subdominios cableados al cert del panel (p.ej. mta-sts) se añaden solos por rol. Solo se incluyen los que caen bajo una zona gestionada (validación DNS-01).', 'Bulwark Config', 'false');
